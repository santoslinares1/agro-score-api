...
describe('ScheduledAnalysisRunnerService', () => {
  ...
  let scheduleRepository: { find: jest.Mock; findOne: jest.Mock; update: jest.Mock };
  let runRepository: { find: jest.Mock; findOne: jest.Mock; create: jest.Mock; save: jest.Mock };
  let fieldsService: jest.Mocked<Pick<FieldsService, 'findOne' | 'findByIdOrFail'>>;
  let analysisService: jest.Mocked<Pick<AnalysisService, 'runFieldAnalysis' | 'findOne' | 'findByField'>>;
  let usersService: jest.Mocked<Pick<UsersService, 'findById'>>;
  let emailService: jest.Mocked<Pick<EmailService, 'sendScheduledAnalysisEmail'>>;
  let configService: { get: jest.Mock };
  let weeklySnapshotService: jest.Mocked<Pick<WeeklyAnalysisSnapshotService, 'createFromAnalysis' | 'findByScheduledRunId'>>;
  ...
  const buildAnalysis = (overrides: Partial<Analysis> = {}): Analysis =>
    ({ id: 'analysis-1', status: 'Procesando', errorMessage: null, ...overrides }) as Analysis;

  const buildAnalysisSummary = (overrides: Partial<FieldAnalysisSummary> = {}): FieldAnalysisSummary =>
    ({ id: 'other-analysis-1', status: 'Procesando', ...overrides }) as FieldAnalysisSummary;

  const DEFAULT_BASE_URL = 'https://app.agroscore.test';

  const buildUser = (overrides: Partial<User> = {}): User =>
    ({ id: 'user-A', email: 'user@example.com', fullName: 'Ana Productora', ...overrides }) as User;

  const buildSnapshot = (overrides: Partial<WeeklyAnalysisSnapshot> = {}): WeeklyAnalysisSnapshot =>
    ({
      id: 'snapshot-1',
      fieldId: 'field-1',
      userId: 'user-A',
      ...
      comparisonVsPrevious: { summary: ['Primer reporte semanal disponible para este campo.'] },
      ...overrides,
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ScheduledAnalysisRunnerService,
        {
          ...
          useValue: { find: jest.fn().mockResolvedValue([]), findOne: jest.fn(), update: jest.fn().mockResolvedValue(undefined) },
        },
        {
          ...
          useValue: {
            ...
            save: jest.fn((data) => Promise.resolve({ id: data.id ?? 'run-1', ...data })),
          },
        },
        { provide: FieldsService, useValue: { findOne: jest.fn(), findByIdOrFail: jest.fn() } },
        {
          provide: AnalysisService,
          useValue: { runFieldAnalysis: jest.fn(), findOne: jest.fn(), findByField: jest.fn() },
        },
        { provide: UsersService, useValue: { findById: jest.fn() } },
        { provide: EmailService, useValue: { sendScheduledAnalysisEmail: jest.fn() } },
        {
          ...
          useValue: { get: jest.fn((key: string) => (key === 'APP_PUBLIC_URL' ? DEFAULT_BASE_URL : undefined)) },
        },
        {
          provide: WeeklyAnalysisSnapshotService,
          useValue: { createFromAnalysis: jest.fn(), findByScheduledRunId: jest.fn() },
        },
      ],
    }).compile();
    ...
    weeklySnapshotService.findByScheduledRunId.mockResolvedValue(buildSnapshot());
  });
  ...
  describe('processDueSchedules', () => {
    ...
    it('un fallo en un schedule no frena el resto', async () => {
      const broken = buildSchedule({ id: 'schedule-broken', fieldId: 'field-broken' });
      const healthy = buildSchedule({ id: 'schedule-healthy', fieldId: 'field-healthy' });
      ...
      await expect(service.processDueSchedules(new Date())).resolves.toBeUndefined();
  ...
  describe('triggerRun', () => {
    ...
    it('FASE 4D-fix: usa la ventana semanal (hoy - 7 días), NO field.startDate/endDate', async () => {
      ...
      runRepository.findOne.mockResolvedValue(null);
      fieldsService.findOne.mockResolvedValue(buildField({ startDate: '2019-01-01', endDate: '2020-01-01' }));
      analysisService.runFieldAnalysis.mockResolvedValue(buildAnalysis());

      await service.triggerRun(schedule, now);

      const expectedRange = computeScheduledAnalysisDateRange(now, { timezone: 'America/Argentina/Cordoba' });
      expect(analysisService.runFieldAnalysis).toHaveBeenCalledWith(
        'field-1',
        expect.objectContaining({ startDate: expectedRange.startDate, endDate: expectedRange.endDate }),
        'user-A',
      );
      expect(analysisService.runFieldAnalysis).not.toHaveBeenCalledWith(
        'field-1',
        expect.objectContaining({ startDate: '2019-01-01', endDate: '2020-01-01' }),
        'user-A',
      );
    });
    ...
    it('FASE 4D: guarda la ventana usada en run.metadata', async () => {
      ...
      const expectedRange = computeScheduledAnalysisDateRange(now, { timezone: schedule.timezone });
    ...
    it('FASE 4D: fuerza includeMapAssets/includeIndexImages/includeImageSeries=true aunque el schedule tenga false', async () => {
      const schedule = buildSchedule({ includeMapAssets: false, includeIndexImages: false, includeImageSeries: false });
      runRepository.findOne.mockResolvedValue(null);
      fieldsService.findOne.mockResolvedValue(buildField());
      analysisService.runFieldAnalysis.mockResolvedValue(buildAnalysis());
      ...
      expect(analysisService.runFieldAnalysis).toHaveBeenCalledWith(
        'field-1',
        expect.objectContaining({ includeMapAssets: true, includeIndexImages: true, includeImageSeries: true }),
        'user-A',
      );
    });
    ...
    it('no duplica una corrida ya existente para la misma semana (mismo scheduleId+scheduledFor)', async () => {
      ...
      const result = await service.triggerRun(schedule, new Date('2026-08-24T12:00:00Z'));
    ...
    it('marca el run failed si AnalysisService.runFieldAnalysis rechaza', async () => {
      ...
      runRepository.findOne.mockResolvedValue(null);
      fieldsService.findOne.mockResolvedValue(buildField());
      analysisService.runFieldAnalysis.mockRejectedValue(new Error('El worker no responde.'));

      const run = await service.triggerRun(schedule, new Date('2026-08-24T12:00:00Z'));
    ...
    it('FIX CRÍTICO: si ya hay un Analysis Procesando ajeno para el campo, no lo llama y no se le ata', async () => {
      ...
      fieldsService.findOne.mockResolvedValue(buildField());
      analysisService.findByField.mockResolvedValue([
        buildAnalysisSummary({ id: 'manual-analysis-ajeno', status: 'Procesando' }),
      ]);
      ...
      expect(run.errorMessage).toContain('Ya hay un análisis en proceso para este campo');
    });
    ...
    it('FIX CRÍTICO: no manda email cuando se rechazó por análisis Procesando ajeno', async () => {
      ...
      runRepository.findOne.mockResolvedValue(null);
      fieldsService.findOne.mockResolvedValue(buildField());
      analysisService.findByField.mockResolvedValue([buildAnalysisSummary({ status: 'Procesando' })]);
    ...
    it('FIX: la carrera de unique(scheduleId, scheduledFor) devuelve la corrida existente en vez de un 500', async () => {
      ...
      runRepository.findOne.mockResolvedValueOnce(null).mockResolvedValueOnce(winnerRun);
    ...
    it('un error de DB que NO es unique violation se sigue propagando tal cual (no se confunde con la carrera)', async () => {
      const schedule = buildSchedule();
      runRepository.findOne.mockResolvedValue(null);
      runRepository.save.mockRejectedValueOnce(new Error('connection terminated'));

      await expect(service.triggerRun(schedule, new Date('2026-08-24T12:00:00Z'))).rejects.toThrow(
    ...
    it('recalcula nextRunAt al terminar (tanto en éxito como en fallo)', async () => {
      ...
      expect((nextRunAtCall as any)[1].nextRunAt).toBeInstanceOf(Date);
      expect((nextRunAtCall as any)[1].nextRunAt.getTime()).toBeGreaterThan(
  ...
  describe('reconcilePendingRuns / envío de email', () => {
    it('envía el email solo cuando el Analysis está Finalizado', async () => {
      ...
      analysisService.findOne.mockResolvedValue(buildAnalysis({ status: 'Finalizado' }));
      fieldsService.findByIdOrFail.mockResolvedValue(buildField());
      usersService.findById.mockResolvedValue(buildUser());
      emailService.sendScheduledAnalysisEmail.mockResolvedValue({ sent: true, provider: 'resend', dryRun: false });

      await service.reconcilePendingRuns();

      expect(emailService.sendScheduledAnalysisEmail).toHaveBeenCalledWith(
        'user@example.com',
        expect.objectContaining({ fieldName: 'Campo A', userName: 'Ana Productora' }),
      );
    });
    ...
    it('FASE 5: NO crea snapshot si el Analysis falló', async () => {
      const run = buildRun({ status: 'processing', analysisId: 'analysis-1' });
      runRepository.find.mockResolvedValue([run]);
      analysisService.findOne.mockResolvedValue(buildAnalysis({ status: 'Error', errorMessage: 'Fallo worker' }));
    ...
    it('FASE 5: el email usa weekStart/weekEnd/summary/dataQualityStatus del snapshot, no una promesa fija', async () => {
      const run = buildRun({ status: 'completed', analysisId: 'analysis-1', emailSentAt: null });
      runRepository.find.mockResolvedValue([run]);
      fieldsService.findByIdOrFail.mockResolvedValue(buildField());
      usersService.findById.mockResolvedValue(buildUser());
      weeklySnapshotService.findByScheduledRunId.mockResolvedValue(
        buildSnapshot({
          ...
          hasRgbImage: false,
          comparisonVsPrevious: { summary: ['El score subió 4 puntos respecto de la semana anterior.'] },
        }),
      );
      emailService.sendScheduledAnalysisEmail.mockResolvedValue({ sent: true, provider: 'resend', dryRun: false });
    ...
    it('no envía email si el Analysis falló (status=Error)', async () => {
      ...
      expect(runRepository.save).toHaveBeenCalledWith(
        expect.objectContaining({ status: 'failed', errorMessage: 'Fallo worker' }),
      );
    });
    ...
    it('no hace nada mientras el Analysis sigue Procesando', async () => {
      const run = buildRun({ status: 'processing', analysisId: 'analysis-1' });
      runRepository.find.mockResolvedValue([run]);
      analysisService.findOne.mockResolvedValue(buildAnalysis({ status: 'Procesando' }));
    ...
    it('no reintenta el email si ya se mandó (emailSentAt seteado) — nunca dos mails para el mismo run', async () => {
      const run = buildRun({ status: 'completed', analysisId: 'analysis-1', emailSentAt: new Date() });
      runRepository.find.mockResolvedValue([run]);
    ...
    it('FIX: no manda email si el schedule fue desactivado antes de completar la corrida', async () => {
      const run = buildRun({ status: 'completed', analysisId: 'analysis-1', emailSentAt: null });
      runRepository.find.mockResolvedValue([run]);
      scheduleRepository.findOne.mockResolvedValue(buildSchedule({ enabled: false }));

      await service.reconcilePendingRuns();

      expect(emailService.sendScheduledAnalysisEmail).not.toHaveBeenCalled();
      expect(runRepository.save).toHaveBeenCalledWith(
        expect.objectContaining({ status: 'failed', errorMessage: expect.stringContaining('desactivado') }),
      );
    });
    ...
    it('FIX: releído el schedule sigue activo, el email se manda con normalidad', async () => {
      const run = buildRun({ status: 'completed', analysisId: 'analysis-1', emailSentAt: null });
      runRepository.find.mockResolvedValue([run]);
      scheduleRepository.findOne.mockResolvedValue(buildSchedule({ enabled: true }));
      fieldsService.findByIdOrFail.mockResolvedValue(buildField());
      usersService.findById.mockResolvedValue(buildUser());
      emailService.sendScheduledAnalysisEmail.mockResolvedValue({ sent: true, provider: 'resend', dryRun: false });
    ...
    it('FIX: sin APP_PUBLIC_URL ni FRONTEND_URL, no manda un email con link relativo (reintenta después)', async () => {
      ...
      expect(runRepository.save).not.toHaveBeenCalledWith(expect.objectContaining({ status: 'failed' }));
    });

    it('un fallo reconciliando un run no frena la reconciliación del resto', async () => {
      const broken = buildRun({ id: 'run-broken', status: 'processing', analysisId: 'analysis-broken' });
      const healthy = buildRun({ id: 'run-healthy', status: 'processing', analysisId: 'analysis-healthy' });
  ...
  describe('runNow', () => {
    it('rechaza un campo ajeno', async () => {
      ...
      await expect(service.runNow('field-1', 'user-B')).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('rechaza si el campo no tiene schedule configurado', async () => {
      ...
      await expect(service.runNow('field-1', 'user-A')).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('dispara triggerRun forzando el informe completo (fase 4D), sin importar los flags guardados', async () => {
      ...
      scheduleRepository.findOne.mockResolvedValue(
        buildSchedule({ includeMapAssets: false, includeIndexImages: false, includeImageSeries: false }),
      );
      runRepository.findOne.mockResolvedValue(null);
      analysisService.runFieldAnalysis.mockResolvedValue(buildAnalysis());
    ...
    it('FASE 4D: "Ejecutar ahora" usa la misma ventana móvil que el dispatcher automático', async () => {
      fieldsService.findOne.mockResolvedValue(buildField({ startDate: '2019-01-01', endDate: '2020-01-01' }));
      scheduleRepository.findOne.mockResolvedValue(buildSchedule({ timezone: 'America/Argentina/Cordoba' }));
      runRepository.findOne.mockResolvedValue(null);
      analysisService.runFieldAnalysis.mockResolvedValue(buildAnalysis());
      ...
      const expectedRange = computeScheduledAnalysisDateRange(before, { timezone: 'America/Argentina/Cordoba' });
