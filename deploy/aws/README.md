...
- ~~**`/auth/register` admin-only:**~~ **Resuelto en SEC-002 (`AUTH-POLICY-1`):** `POST /auth/register` fue eliminado del backend — el alta de un usuario nuevo pasa exclusivamente por invitación aceptada o creación administrativa (`POST /admin/users`). Ver `docs/admin-backend.md` en `agro-score-api`.
